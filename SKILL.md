...
|---------------|----------------|----------------|
| `/cue <主题>` | 智能调研（自动匹配研究视角）/ Smart research (auto-matches perspective) | 5-30 分钟 / mins |
| `/cue --mode <模式> <主题>` | 指定视角深度研究 / Deep research with specific perspective | 5-30 分钟 / mins |
...
**v1.0.5 Node.js 架构 / Node.js Architecture:**
...
│   └── backgroundExecutor.js # 后台任务执行器 / Background task executor
├── api/
│   └── cuecueClient.js   # API 客户端 / API client
├── cron/                 # 定时任务 / Cron jobs
│   ├── research-worker.js # 研究任务工作器 / Research task worker
│   ├── monitor-daemon.js  # 监控守护进程 / Monitor daemon
│   └── run-check.js       # 检查脚本入口 / Check script entry
├── notifier/             # 通知推送 / Notification
│   └── index.js          # 统一通知模块 / Unified notification module
├── utils/                # 工具函数 / Utilities
│   ├── fileUtils.js      # 文件操作 / File operations
│   ├── envUtils.js       # 环境变量（安全存储）/ Environment (secure storage)
│   ├── validators.js     # 验证工具 / Validators
│   ├── notificationQueue.js # 可靠通知队列 / Reliable notification queue
│   └── smartTrigger.js   # 智能触发评估 / Smart trigger evaluation
├── commands/             # 命令处理器（预留）/ Command handlers (reserved)
└── executors/            # 执行引擎（预留）/ Execution engines (reserved)
```
...
### v1.0.5 (2026-02-26)
- 🔧 修复：恢复缺失的 notifier 模块（通知推送功能）
- 🔧 修复：恢复缺失的 cron 模块（research-worker.js, monitor-daemon.js）
- 🔧 修复：backgroundExecutor.js 中未定义变量 bug
- 🔧 修复：monitor-daemon.js async 函数声明缺失
- 🔧 重构：API 客户端使用官方流式 SSE 协议（参考 cuecue-ts-sdk）
- ✅ 优化：研究耗时描述更新为 5-30 分钟（超时保持 60 分钟）
- ✅ 优化：通知系统可靠性（本地队列 + OpenClaw CLI）
- ✅ 优化：错误处理和日志记录
